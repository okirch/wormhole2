...
buffer_read_file(const char *filename, int flags)
{
	...
	if ((fd = open(filename, O_RDONLY)) < 0) {
		log_fatal("Unable to open file %s: %m\n", filename);
	}

	if (fstat(fd, &stb) < 0)
		log_fatal("Cannot stat %s: %m\n", display_name);

	bp = buffer_alloc_write(stb.st_size);
	if (bp == NULL)
		log_fatal("Cannot allocate buffer of %lu bytes for %s: %m\n",
				(unsigned long) stb.st_size,
				display_name);

	count = read(fd, bp->data, stb.st_size);
	if (count < 0)
		log_fatal("Error while reading from %s: %m\n", display_name);

	if (count != stb.st_size)
		log_fatal("Short read from %s\n", display_name);

	if (closeit)
		close(fd);

	log_debug("Read %u bytes from %s\n", count, display_name);
	bp->wpos = count;
	return bp;
}
