...
static inline bool
__buffer_advance_head(buffer_t *bp, unsigned int count)
{
	if (count > bp->wpos - bp->rpos)
		return false;
...
static inline bool
buffer_consume_padding(buffer_t *bp, size_t size)
{
	size_t over = bp->rpos % size;

	if (over == 0)
		return true;

	return buffer_skip(bp, size - over);
}
...
static inline bool
__buffer_advance_tail(buffer_t *bp, unsigned int count)
{
	if (count > bp->size - bp->wpos)
		return false;
...
static inline bool
buffer_put_padding(buffer_t *bp, size_t size)
{
	static unsigned char zeros[16];
	size_t over = bp->wpos % size;
	...
	return buffer_put(bp, zeros, size - over);
}
