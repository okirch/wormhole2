...
wormhole_layer_new(const char *name, const char *path, unsigned int depth)
{
	...
	layer->config_path = concat_path(layer->path, "layer.conf");
...
wormhole_layer_load_config(struct wormhole_layer *layer)
{
	...
	while (fgets(linebuf, sizeof(linebuf), fp) != NULL) {
		...
		if (!strcmp(kwd, "use-layer")) {
			if (layer->nused >= LOWER_LAYERS_MAX) {
				log_error("%s:%u: too many lower layers",
						layer->config_path, line);
				okay = false;
			} else {
				layer->used[layer->nused++] = strdup(value);
			}
		} else {
